...
# class Timecalculate:
#     img_data, img_data_np, img_data_ls = [],[],[]
...
def time_check_with_numpy():
    ...
    img_data_np = np.empty([1,224,224,3], dtype='float32')
    start_time = datetime.now()
    for i in df['image']:
        image = load_img(str(i) + '.jpg', target_size=(224,224))
        image = img_to_array(image)
        ...
        img_data_np = np.append(img_data_np, image,axis=0)
        # img_data_np = np.append(img_data_np,image)
    img_data_np = np.delete(img_data_np,0,0) # (arr,index,axis)
...
'''  
'''
In bigger dataset I tried in kaggle notebook where numpy method is significantly slower than
the other twos! 
'''